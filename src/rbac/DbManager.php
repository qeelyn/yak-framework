...
class DbManager extends \yii\rbac\DbManager
{
    ...
    /**
     * 权限检查入口,如果权限项不在permission集合,则返回true,这是与YII比较不同的做法
     *
    ...
    public function loadFromCache()
    {
        ...
        //权限项相关数据获取
        //1.permission
        $query = (new Query())->select('id,name,rule_id,description')->from($this->itemTable)
            ->where(['status' => 1]);
        $this->items = [];
        $this->nameMappers = [];
        ...
        $query = (new Query())->select('id,name,rule_id,description')->from($this->roleItemTable)
            ->where(['status' => 1]);
